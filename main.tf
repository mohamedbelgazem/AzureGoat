...
# Use existing Automation Account (replace <existing-account-name> with actual name)
data "azurerm_automation_account" "existing" {
  name                = "<existing-account-name>"
  resource_group_name = var.resource_group
}

# Uncomment below if you need to create a new Automation Account in a different region
/*
resource "azurerm_automation_account" "dev_automation_account_test" {
  ...
  location            = "westus" # Use a different region
  resource_group_name = var.resource_group
  sku_name            = "Basic"
  identity {
    type         = "UserAssigned"
